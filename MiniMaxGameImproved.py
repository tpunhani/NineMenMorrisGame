...
class MiniMaxGameImproved:
    '''
    Program plays a move in the midgame/endgame phase of the game with improved static estimation function.
    '''
    ...
    def staticEstimation(self, boardPos):
        ...
        numWhiteMills = morrisVariantUtil.countMills(boardPos, 'W')
        ...
        if numBlackPieces <= 2:
            ...
            return ((1000*((numWhiteMills + numWhitePieces) - (numBlackPieces))) - numBlackMoves)
...
boardPosition = morrisVariantUtil.readInputFile(miniMaxGameImproved.inFile)
generatedPosition = miniMaxGameImproved.maxMin(boardPosition, miniMaxGameImproved.depth)
print(f'Input position: {boardPosition} Output position: {generatedPosition}')
print(f'Position evaluated by static estimation: {miniMaxGameImproved.positionsEvaluated}')
print(f'MINIMAX Estimate: {miniMaxGameImproved.minimax_estimate}')
morrisVariantUtil.writeOutputFile(miniMaxGameImproved.outFile, generatedPosition)
miniMaxGameImproved.checkForWin()